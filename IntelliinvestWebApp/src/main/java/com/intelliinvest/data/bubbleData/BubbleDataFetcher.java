...
public class BubbleDataFetcher {
	...
	public List<BubbleDataResponse> getTechnicalBubbleData() {
		logger.info("Inside getTechnicalBubbleData()...");

		Aggregation aggregation = newAggregation(
		/* match(Criteria.where("signalDate").gte(date)), */
		/* group("signalDate","aggSignal"), */
		group("signalDate", "aggSignal").count().as("count"),
				project("signalDate", "count").and("aggSignal"),
				sort(Sort.Direction.ASC, "signalDate"));
		AggregationResults<BubbleData> results = mongoTemplate.aggregate(
				aggregation, COLLECTION_STOCK_SIGNALS, BubbleData.class);
		List<BubbleData> bubbleDatas = results.getMappedResults();

		return getBubbleDataResponse(bubbleDatas);
	}

	private List<BubbleDataResponse> getBubbleDataResponse(
			List<BubbleData> bubbleDatas) {
...
//		List<BubbleDataResponse> bubbleDataResponses = new ArrayList<>();
		...
		for (BubbleData bubbleData : bubbleDatas) {
			...
			if (bubbleData.getAggSignal() != null
					&& (bubbleData.getAggSignal().equals(
							IntelliinvestConstants.BUY) || bubbleData
